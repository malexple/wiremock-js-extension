...
- [Агрегатные функции (Фаза 4c)](#агрегатные-функции-фаза-4c)
- [Цикл forEach (Фаза 5)](#цикл-foreach-фаза-5)
...
Язык поддерживает переменные, условия, циклы forEach, арифметику, логические операторы, вызовы whitelisted-функций и возврат JSON-объекта.
...
| `for ... of` | `for (var item of items) { ... }` | Итерация по массиву, только для поиска с последующим `return`, см. ниже |
...
**Важно:** язык не поддерживает переприсваивание уже объявленной переменной без `var` (`total = total + 1;` — синтаксическая ошибка, `ScriptParseException`). Для аккумуляции значений (суммы, счётчики) используйте встроенные агрегатные функции — `sum()`, `count()`, `avg()` — вместо ручного накопления в цикле, см. раздел [Агрегатные функции](#агрегатные-функции-фаза-4c).
...
## Агрегатные функции (Фаза 4c)

Для подсчёта суммы, количества и среднего значения по массиву данных из тела запроса используются встроенные агрегатные функции — они закрывают задачи аккумуляции без необходимости в ручном цикле с переприсваиванием переменной (которое язык сознательно не поддерживает, см. [Переменные](#переменные)).

| Функция | Сигнатура | Возвращает | Описание |
|---|---|---|---|
| `sum(array)` | `sum(Array) -> Number` | Сумма всех числовых элементов массива | Пустой массив даёт `0` |
| `count(array)` | `count(Array) -> Number` | Количество элементов массива | Пустой массив даёт `0` |
| `avg(array)` | `avg(Array) -> Number` | Среднее арифметическое элементов массива | Пустой массив бросает `ScriptExecutionException` (деление на ноль) |
...
Агрегатные функции принимают на вход результат `jsonField()` с JSONPath, извлекающим массив (например, `$.items[*].price`), а не сам объект целиком — так интерпретатор сразу получает готовый список чисел для подсчёта.

## Цикл forEach (Фаза 5)

`for (var item of items)` — это строгий итератор по уже готовому массиву. Количество итераций всегда равно размеру массива, программист не может повлиять на него, поэтому классическая проблема "бесконечного цикла" здесь архитектурно исключена — а таймаут исполнения в 100 мс остаётся дополнительной страховкой на уровне всего скрипта.

**Важное ограничение дизайна:** цикл предназначен только для поиска и фильтрации с немедленным `return` при найденном условии — 95% сценариев мокирования сводятся именно к этому паттерну. Аккумуляция значений (суммы, счётчики) через ручной цикл не поддерживается, потому что язык не разрешает переприсваивание переменных без `var` — для этого используйте встроенные функции `sum()`, `count()`, `avg()` (см. выше).
...
Переменная итератора (`item` в примере выше) действует только внутри тела цикла — до и после цикла она либо отсутствует, либо восстанавливает своё прежнее значение, если переменная с тем же именем уже была объявлена раньше в скрипте (временное перекрытие, shadowing).

### Комбинация с агрегатными функциями

Типичный паттерн — заранее посчитать агрегат по всему массиву через `sum()`/`count()`/`avg()`, а `forEach` использовать исключительно для поиска конкретного элемента:
...
### Пример 6 — фильтрация заказов через forEach и агрегаты
...
curl -X POST http://localhost:8888/__admin/extensions/wiremock-js/scripts \
  ...
  -d '{
    ...
    "description": "Находит VIP-заказ в списке и одновременно считает общую сумму по всем заказам",
...
| Таймаут исполнения | 100 мс | Защита от бесконечных циклов / зависаний. `forEach` итерирует строго по размеру готового массива и не может зависнуть сам по себе, но общий таймаут остаётся страховкой на случай сложных вложенных вычислений |
...
| Переприсваивание переменных | Не поддерживается | `total = total + 1;` без `var` — синтаксическая ошибка; используйте `sum()`/`count()`/`avg()` для аккумуляции |
| Whitelist функций | `query, header, body, method, pathSegment, jsonField, contains, random, now, nowPlusDays, uuid, randomInt, matches, sum, count, avg` | Скрипт не имеет доступа ни к чему за пределами этого списка |
